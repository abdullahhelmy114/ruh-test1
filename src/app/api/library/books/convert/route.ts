...
export async function POST(req: Request) {
  ...
  try {
    ...
    const options = { density: 150, saveFilename: bookId, savePath: "/tmp", format: "png", width: 800, height: 1100 };
    const converter = fromBuffer(pdfBuffer, options);
    const result = await converter.bulk(-1);

    if (!result || !Array.isArray(result)) throw new Error("Conversion failed");
    ...
    for (let i = 0; i < result.length; i++) {
      ...
      const webpBuffer = await sharp(pageBuffer).webp({ quality: 80 }).toBuffer();

      const driveUrl = await uploadFileToGoogleDrive(webpBuffer, `page-${i + 1}.webp`, "image/webp");
      const imageUrl = driveUrlToCdnUrl(driveUrl);

      pages.push({ book_id: bookId, page_number: i + 1, image_url: imageUrl });
