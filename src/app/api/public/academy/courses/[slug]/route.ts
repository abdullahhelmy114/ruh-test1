...
// Public course page data: published outline titles and upcoming classes.
const LIMIT = { limit: 120, windowMs: 10 * 60 * 1000 };

export const GET = withApi<{ slug: string }>(async (req, ctx) => {
  const check = checkRateLimit(`public-course:${clientKey(req)}`, LIMIT);
  if (!check.allowed) {
    return NextResponse.json({ error: "Too many requests" }, { status: 429, headers: rateLimitHeaders(retryAfterSeconds(check)) });
  }
  const { slug } = await ctx.params;
  return NextResponse.json({ data: await publicService.course(slug) }, { headers: PUBLIC_SHORT_CACHE });
});
