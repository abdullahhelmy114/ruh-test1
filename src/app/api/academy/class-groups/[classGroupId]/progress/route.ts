...
// Learners: their own progress. Teachers and administrators: ?learnerUid= names the TARGET learner.
export const GET = withApi<{ classGroupId: string }>(async (req, ctx) => {
  const user = await requireAuth(req);
  const { classGroupId } = await ctx.params;
  const data = await progressService.progress(user, classGroupId, { learnerUid: readStringParam(req, "learnerUid") });
