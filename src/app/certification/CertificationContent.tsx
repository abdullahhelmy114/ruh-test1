  ...
  return (
    <div className="min-h-screen bg-background">
          ...
          <div className="relative z-10">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              {[
                ...
                <div key={i} className="flex items-start gap-3 bg-white/50 rounded-2xl p-4 border border-gray-200/50">
                  <CheckCircle size={20} className="text-emerald-500 mt-0.5 shrink-0" />
                  <span className="text-sm text-gray-500"><T>{step}</T></span>
                </div>
              ))}
            </div>
      ...
      <section className="mx-auto max-w-3xl px-4 py-16 md:px-8">
        <div className="glass relative overflow-hidden rounded-3xl border border-amber-500/30 bg-gradient-to-br from-gold/5 to-transparent p-8 shadow-elegant">
          <div className="absolute -right-10 -top-10 h-24 w-24 rounded-full bg-amber-500/20 blur-2xl" />
          <div className="relative z-10 flex flex-col sm:flex-row items-start gap-5">
            <div className="grid h-12 w-12 place-items-center rounded-2xl bg-amber-500/20 text-gold shrink-0">
        ...
        <div className="glass rounded-3xl border border-gray-200/50 bg-card p-8 md:p-12 shadow-elegant">
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <input
              ...
              className="flex-1 rounded-2xl border border-gray-200 bg-background px-4 py-3 text-sm outline-none focus:border-amber-500 focus:ring-2 focus:ring-amber-500/20"
            />
            <button
              onClick={handleVerify}
              disabled={verifyStatus === "loading"}
              className="inline-flex items-center justify-center gap-2 rounded-full bg-amber-500 px-6 py-3 text-sm font-semibold text-black shadow-gold transition hover:scale-[1.02] disabled:opacity-50"
            >
