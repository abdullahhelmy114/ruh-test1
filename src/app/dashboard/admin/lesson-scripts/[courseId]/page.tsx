    ...
    <div className="min-h-screen bg-background text-foreground selection:bg-primary/20 selection:text-primary flex">
      ...
      {view === "editor" && (
        <aside className="fixed left-4 top-1/2 -translate-y-1/2 flex flex-col gap-3 p-3 bg-card/90 backdrop-blur-xl border border-border rounded-3xl shadow-2xl z-50">
          <button onClick={() => setActiveTab("format")} className={`p-3 rounded-2xl transition-all ${activeTab === "format" ? "bg-primary text-primary-foreground shadow-md" : "text-muted-foreground hover:bg-secondary hover:text-foreground"}`} title="Formatting">
            <PenTool size={22} />
          </button>
          <button onClick={() => setActiveTab("ai")} className={`p-3 rounded-2xl transition-all ${activeTab === "ai" ? "bg-primary text-primary-foreground shadow-md" : "text-muted-foreground hover:bg-secondary hover:text-foreground"}`} title="AI Tools">
            <BrainCircuit size={22} />
          </button>
          <button onClick={() => setActiveTab("insert")} className={`p-3 rounded-2xl transition-all ${activeTab === "insert" ? "bg-primary text-primary-foreground shadow-md" : "text-muted-foreground hover:bg-secondary hover:text-foreground"}`} title="Insert Media & Files">
            <Blocks size={22} />
          </button>
          <div className="w-8 h-px bg-border mx-auto my-1" />
          <button onClick={() => setIsLibraryOpen(true)} className="p-3 rounded-2xl text-accent-foreground bg-accent/10 hover:bg-accent/20 transition-all" title="Library & Models">
            <Library size={22} />
          </button>
        </aside>
      )}
        ...
        {view === "editor" && (
          <section className="relative min-h-screen bg-muted/20 animate-in zoom-in-95 duration-300">
            ...
            <nav className="sticky top-4 mx-auto max-w-4xl z-40 flex items-center justify-between bg-card/95 backdrop-blur-2xl border border-border px-5 py-2.5 rounded-full shadow-xl transition-all duration-300">
              ...
              {activeTab === "insert" && (
                <div className="flex items-center gap-1 animate-in fade-in px-2">
                  ...
                  <label className="p-2 rounded-lg hover:bg-secondary cursor-pointer" title="Import PDF as text">
                    <FileDigit size={16} />
                    <input type="file" accept=".pdf" onChange={handlePdfToText} className="hidden" />
                  </label>
                  <label className="p-2 rounded-lg hover:bg-secondary cursor-pointer" title="Upload Video">
                    <FileUp size={16} />
            ...
            <article className="pt-12 pb-32 px-4 flex justify-center">
              <div className="w-full max-w-4xl min-h-[800px] bg-card border border-border shadow-2xl rounded-3xl p-12 md:p-16 relative">
                ...
                <input
                  ...
                  className="w-full text-4xl font-serif font-bold text-foreground bg-transparent outline-none placeholder:text-muted-foreground/30 border-b border-transparent hover:border-border focus:border-primary pb-4 mb-6 transition"
                />
