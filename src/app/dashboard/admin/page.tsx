...
const TABS = [
  ...
  { key: "manage-course", label: "Manage course", icon: BookOpen },
  { key: "teachers", label: "Teacher Verification", icon: ShieldCheck },
  { key: "course", label: "Course Moderation", icon: BookOpen },
  ...
  { key: "model-course", label: "Model course", icon: BookOpen },
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-10 md:px-8">
      ...
      <div className="mt-8">
        ...
        {tab === "manage-course" && <ManagecourseTab />}
        {tab === "teachers" && <TeacherVerificationTab />}
        {tab === "course" && <CourseModerationTab />}
        ...
        {tab === "model-course" && <ModelcourseTab />}
              ...
              {course.map((c: any) => (
                <tr key={c.id} className="border-t border-border">
                  ...
                  <td className="p-3 flex gap-2">
                    <button
                      onClick={() => router.push(`/dashboard/admin/course/${c.id}/edit`)}
...
// Teacher applications moved to the academy administration, where approval
// activates the account in one guarded transaction and every decision has a
// history. The old tab's approve button called a route that updated a table
// the application does not have, and its reject button only hid the card.
function TeacherVerificationTab() {
  return (
    <div className="space-y-4 rounded-3xl border bg-card p-8 shadow-elegant">
      <h2 className="font-serif text-2xl">
        <T>Pending Teacher Applications</T>
      </h2>
      <p className="text-muted-foreground">
        <T>Teacher applications are reviewed in the academy administration.</T>
      </p>
      <Link href="/academy/manage/teachers" className="inline-flex rounded-full bg-primary px-6 py-2 text-sm font-semibold text-primary-foreground hover:bg-primary/90">
...
function CourseModerationTab() {
    ...
    user.getIdToken().then(async (token) => {
      try {
        const [courseRes, lessonsRes] = await Promise.all([
          fetch("/api/admin/pending-course", {
            headers: { Authorization: `Bearer ${token}` },
          }),
  ...
  const totalPending = course.length + lessons.length;
...
function PagesTab() {
  ...
  const handleSave = async () => {
    if (!selectedSlug || !title.trim() || !content.trim() || !user) return;
    setSaving(true);
    const token = await user.getIdToken();
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          ...
          >
            {submitting ? (
              ...
              <>
                <Package size={14} /> <T>Create Bundle</T>
              </>
            )}
          </button>
        </div>
...
function ApplicationsTab() {
  ...
  const fetchApplications = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (res.ok) setApplications(Array.isArray(data) ? data : data.applications || []);
    } catch {
