...
export default function AdmincoursePage() {
  ...
  const handleDelete = async (id: string) => {
    ...
    await fetch(`/api/admin/course/id`, { method: "DELETE" });
    fetchcourse();
  };
  ...
  return (
    <div className="max-w-6xl mx-auto p-6">
      <div className="flex justify-between items-center mb-6">
        ...
        <Link href="/dashboard/admin/course/new" className="bg-primary text-primary-foreground rounded-full px-4 py-2 flex items-center gap-2"><Plus size={18} /> <T>كورس جديد</T></Link>
      </div>
            ...
            {course.map(c => (
              <tr key={c.id} className="border-t border-border">
                ...
                <td className="p-3 flex gap-2">
                  <Link href={`/dashboard/admin/course/${c.id}/edit`} className="p-1 rounded hover:bg-accent"><Pencil size={16} /></Link>
