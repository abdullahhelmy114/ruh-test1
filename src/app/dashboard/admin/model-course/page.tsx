...
import { useAuth } from "@/lib/firebase/AuthProvider"; // إضافة useAuth
...
export default function AdminModelcoursePage() {
  ...
  const fetchcourse = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setError(data.error || "فشل جلب الكورسات");
      }
    } catch {
      setError("فشل جلب الكورسات");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id: string) => {
    if (!confirm("هل أنت متأكد من حذف هذا النموذج؟") || !user) return;
    setDeletingId(id);
    const token = await user.getIdToken();
            ...
            <Card key={course.id} className="bg-card border-border hover:border-primary/30 transition">
              ...
              <CardContent className="flex items-center justify-between gap-2">
                <Button
                  variant="ghost"
                  size="sm"
                  onClick={() => router.push(`/dashboard/admin/model-course/${course.id}/lessons`)}
