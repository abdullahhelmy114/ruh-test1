...
export default function AvailablecoursePage() {
  ...
  const fetchData = () => {
    Promise.all([
      fetch("/api/teacher/available-course").then((res) => res.json()),
      fetch("/api/teacher/applications").then((res) => res.json()),
    ])
      .then(([courseData, appsData]) => {
        if (Array.isArray(courseData)) setcourse(courseData);
    ...
    <div className="container p-6 space-y-10">
      ...
      <section>
        <h2 className="text-2xl font-semibold text-foreground mb-5">
          الكورسات المتاحة ({course.length})
        </h2>
