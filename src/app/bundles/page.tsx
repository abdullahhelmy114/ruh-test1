      ...
      <div className="grid md:grid-cols-3 gap-6">
        {bundles.map((b) => (
          <div key={b.title} className={`glass rounded-3xl p-6 text-center relative ${b.featured ? "ring-2 ring-amber-500" : ""}`}>
            {b.featured && (
              <span className="absolute -top-3 left-1/2 -translate-x-1/2 bg-amber-500 text-gray-900 text-xs font-bold px-4 py-1 rounded-full">
                <Sparkles size={12} className="inline mr-1" /> <T>Best Value</T>
              </span>
            )}
            <h3 className="font-serif text-xl mt-2"><T>{b.title}</T></h3>
            <p className="text-sm text-gray-500 mt-1">{b.levels}</p>
            <div className="mt-4">
              <span className="font-serif text-3xl font-bold text-amber-700">${b.price}</span>
              <span className="text-sm line-through text-gray-500 ml-2">${b.original}</span>
              <span className="ml-2 bg-emerald-100 text-white text-xs px-2 py-0.5 rounded-full">-{b.discount}%</span>
            </div>
            ...
            <Link href="/marketplace" className="mt-5 inline-block w-full rounded-full bg-amber-500 py-2.5 text-sm font-semibold text-white hover:bg-amber-400">
              <T>Get Started</T>
            </Link>
          </div>
        ))}
      ...
      {!user && (
        <div className="mt-12 text-center">
          <div className="glass rounded-3xl p-8 inline-block">
            ...
            <p className="text-gray-500 mb-6"><T>Join now and get access to exclusive bundles.</T></p>
