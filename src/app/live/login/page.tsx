...
import { useAuth } from "@/lib/firebase/AuthProvider"; // ← أضفنا هذا
...
export default function LoginPage() {
  ...
  const { user, role, isLoading } = useAuth(); // ← جلب الدور بعد تسجيل الدخول

  // توجيه المستخدم حسب الدور بعد تسجيل الدخول
  useEffect(() => {
    if (!user || isLoading) return; // انتظر حتى ينتهي تحميل الجلسة
    if (!role) return; // لا تفعل شيء حتى يتوفر الدور
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // لا حاجة لتوجيه يدوي – useEffect سيتكفل بذلك
    } catch (err: any) {
  ...
  // عرض مؤشر تحميل أثناء تحديد الدور
  if (user && isLoading) {
    return (
