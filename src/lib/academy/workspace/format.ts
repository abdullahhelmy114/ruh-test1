...
export function formatDate(value: string | null | undefined, locale: ProductLocale, timeZone: string | null): string {
  ...
  if (!date) return "";
  if (plain) return new Intl.DateTimeFormat(INTL_LOCALE[locale], { dateStyle: "medium", timeZone: "UTC" }).format(date);
  // An instant still needs the academy's clock to name a calendar day.
  if (timeZone === null) return "";
  return new Intl.DateTimeFormat(INTL_LOCALE[locale], { dateStyle: "medium", timeZone }).format(date);
}
